...
class ChatService:
    ...
    def _build_system_prompt(self) -> str:
        ...
        回答规则：
        1. 直接回答用户问题，提供准确有用的信息
        2. 介绍单个文物时，在末尾添加 [ID:文物ID]；多个文物时不加标签
        3. 回答自然流畅，就像专业导游一样与访客交流
        4. 有相关资料时可以提及（如"根据馆藏资料"），没有资料时直接回答，不要提资料不来源于知识库
        ...
        try:
            ...
            if should_show_card:
                ...
                for doc in relevant_docs:
                    ...
                    if str(doc_id) == str(artifact_id) or (doc_meta.get('name') and answer.find(doc_meta['name']) != -1):
                        artifact_info = doc_meta
                        break
            ...
            if config.USE_MOCK_MODE:
                ...
                if should_show_card:
                    ...
                    for doc in relevant_docs:
                        doc_meta = doc.get('metadata', {})
                        doc_id = doc_meta.get('artifact_id') or doc_meta.get('id')
                        if str(doc_id) == str(artifact_id) or (doc_meta.get('name') and full_answer.find(doc_meta['name']) != -1):
                            artifact_info = doc_meta
                            break
