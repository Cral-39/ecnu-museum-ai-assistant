...
async def chat_endpoint(request: ChatRequest): 
    ...
    if request.stream: 
        return StreamingResponse( 
            generate_stream_response(request.question, lang), 
        ...
        result = chat_service.chat_completion(request.question, lang) 
        return result 


async def generate_stream_response(question: str, lang: str = "zh"): 
    """ 
    生成流式响应的生成器 
    :param question: 用户问题 
    :param lang: 语言 
    :yield: JSON格式的响应片段 
    """ 
    for chunk in chat_service.stream_chat_completion(question, lang): 
...
async def image_recognition_endpoint(file: UploadFile = File(..., description="要识别的图片文件")): 
    ...
    # 使用默认语言进行图片识别
    result = chat_service.image_recognition(image_base64, mime_type, lang="zh") 
    return result
